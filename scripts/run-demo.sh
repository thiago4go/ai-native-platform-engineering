...
architecture_map() {
  ...
  printf "    [Platform catalog] [Skill registry] [Central agentgateway control plane]\n"
  printf "             |                         [Eval / telemetry / evidence]\n"
  printf "             | route + policy\n"
  printf "             v\n"
  printf "  %sDeveloper device / dev edge%s\n" "${cyan}" "${reset}"
  printf "    [Claude Code] ---- allowed tools ----> %s[Demo MCP server]%s\n" "${blue}" "${reset}"
...
printf "  4. The cluster/device agentgateway topology as architecture\n"
printf "  5. A local route proof artifact\n"
printf "  6. An append-only evidence ledger\n"
architecture_map
